...
// Comment out this define if you want bootloader to wait forever
#define BOOTLOADER_TIMEOUT
...
// How many ms to wait before running applicaation
...
// This gets set to TRUE when we start communicating with the PC, at which time we ignore the timeout
// value.
BOOL	StartedProgramming;

/** DECLARATIONS ***************************************************/
#pragma code

BOOL IsThereAnApplication(void)
{
    // If the first word of memory is programmed, then we have an app
	return (*(int *)BootMemStart != 0xFFFFFFFF);	
}	
...
// This is not based on a real timer, just the number of times it was
// called. Return true if we have timed out
BOOL Timedout(void)
{
	static UINT32 CallCount = 1000000;
	
	if (StartedProgramming)
	{
		return (FALSE);
	}
...
//	}
    ...
    while(!Timedout() || !IsThereAnApplication())
    {
